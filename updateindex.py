...
if __name__ == "__main__":
    ...
    for row in all_index_list.iterrows():
        values = [str(row[0]), row[1].SYMBOL, row[1].NAME_OF_COMPANY,' ',row[1].DATE_OF_LISTING.strftime("%d-%m-%Y"),row[1].SERIES,row[1].ISIN_NUMBER,row[1].FACE_VALUE,row[1].PAID_UP_VALUE,row[1].MARKET_LOT]
